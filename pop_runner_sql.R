...
getwd()
setwd("/Users/sahildawani/Documents/untitled folder")
...
#female consumers are 6903 with average age of 30.61394 
#and male are 2129 with avg age of 32.45186.
################################################################################
...
# Interpret your output in simple English (1-2 lines):
#loyalty_count     average_age       loyalty_status
#1          1529    29.37345              0
#2          1740    30.10345              1
#3          2612    30.69908              2
#4          1385    31.59278              3
...
#The consumer who recieved the pop up message and continued to adding discount
#code to their card are 1487.
#and the consumers who did'nt recieve the pop up message are 4516.
...
# Interpret your output in simple English (1-2 lines):
#on average, the consumers spend 135.2142 during their online purchase.
################################################################################
...
# Interpret your output in simple English (1-2 lines):
# 716 consumers opened the email blast.
######################################################################################################
...
sqldf("
      ...
      pop_up from pop_up, purchase
      WHERE pop_up.consumer_id=purchase.consumer_id
      GROUP BY pop_up
      ")

# Interpret your output in simple English (1-2 lines):
# No, the sum of sales and average sales is greater for consumers who do not recieve the pop up.
######################################################################################################
...
sqldf("
      ...
      ORDER BY sales_amount_total ASC LIMIT 1
      ")
...
# Interpret your output in simple English (1-2 lines):
##The consumer with consumer_id 5887286353 spent lowest during online shopping and did not open the pop up message. 
######################################################################################################
...
sqldf("
      SELECT SUM(sales_amount_total) AS sum_sales,
      AVG(sales_amount_total) AS avg_sales,
      opened_email FROM email,purchase
      WHERE email.consumer_id=purchase.consumer_id
      GROUP BY opened_email
      ")

# Interpret your output in simple English (1-2 lines):
#Yes, as the average sales is high with consumers who opened the email blast.
######################################################################################################
...
# Interpret your output in simple English (1-2 lines):
# The consumer who spent the most from purchase table OPENED the email blast.
######################################################################################################
# Best Luck!
######################################################################################################
